...
async function inicializarFavoritos() {
    try {
        ...
        const resProductos = await fetch('[https://justina-store-backend.onrender.com](https://justina-store-backend.onrender.com)api/productos');
        catalogoCompleto = await resProductos.json();

        // B. Traemos solo los IDs que esta clienta guardó como favoritos
        const resFavs = await fetch(`[https://justina-store-backend.onrender.com](https://justina-store-backend.onrender.com)api/favoritos/${usuarioFav.id}`);
...
async function quitarDeFavoritos(idProducto) {
    try {
        const res = await fetch(`[https://justina-store-backend.onrender.com](https://justina-store-backend.onrender.com)api/favoritos/${usuarioFav.id}/${idProducto}`, {
            method: 'DELETE'
        });
