...
async function inicializarFavoritos() {
    try {
        ...
        const resFavs = await fetch(`http://localhost:3000/api/favoritos/${usuarioFav.id}`);
...
async function quitarDeFavoritos(idProducto) {
    try {
        const res = await fetch(`http://localhost:3000/api/favoritos/${usuarioFav.id}/${idProducto}`, {
            method: 'DELETE'
        });
