...
document.getElementById('form-perfil').addEventListener('submit', async (e) => {
    ...
    try {
        const resp = await fetch(`${API_URL}/api/usuarios/${usuario.id}`, {
            method: 'PUT',
...
document.getElementById('btn-eliminar-cuenta').addEventListener('click', async () => {
    if (confirm("⚠️ ¿Estás totalmente segura de eliminar tu cuenta? Esta acción no se puede deshacer.")) {
        await fetch(`${API_URL}/api/usuarios/${usuario.id}`, { method: 'DELETE' });
...
async function verificarPasswordSeguridad() {
    ...
    try {
        const resp = await fetch(`${API_URL}/api/usuarios/verificar-password`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function cargarDirecciones() {
    ...
    try {
        const res = await fetch(`${API_URL}/api/direcciones/${usuario.id}`);
...
async function eliminarDireccion(idDireccion) {
    ...
    try {
        const resp = await fetch(`${API_URL}/api/direcciones/${idDireccion}`, {
            method: 'DELETE'
        });
        if (resp.ok) {
...
document.getElementById('form-nueva-direccion').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (idEdicion === "") {
            // CREAR NUEVA (POST)
            resp = await fetch(`${API_URL}/api/direcciones`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            resp = await fetch(`${API_URL}/api/direcciones/${idEdicion}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(datosDir)
            });
...
async function cargarMisPedidos() {
    ...
    try {
        const res = await fetch(`${API_URL}/api/mis-pedidos/${usuario.id}`);
...
async function cancelarPedido(idPedido) {
    if (confirm("⚠️ ¿Estás segura de que querés cancelar esta compra? Esta acción no se puede deshacer.")) {
        try {
            const res = await fetch(`${API_URL}/api/pedidos/${idPedido}/cancelar`, {
